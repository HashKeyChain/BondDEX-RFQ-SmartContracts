...
- `US1 - Launch and Subscribe`：平台审批发行，发行人创建债券，配置合规模块，管理员审批认购窗口，做市商或合格参与方完成一级认购
...
| `BondIssuance` | 负责一级市场认购审批、认购窗口管理、赎回资金注入、直接领取与代理领取 |
...
`quoteFee(bondToken, partyA, partyB, quoteAmount)` 提供链上手续费预估查询，做市商可在链下构造订单前调用。每笔订单通过 `maxFeeBps` 字段锁定 maker 签名时的最高费率预期，防止签名后费率被修改导致超预期扣费。
