...
**🔒 AUDIT-FIX(N11) 修订后 / 最小权限初始化**：自审计修复批次起，**`BondFactory` / `BondIssuance` / `RFQSettlement` 的构造函数与 `initialize` 都只发放 `DEFAULT_ADMIN_ROLE`**，不再预先打包发放次级角色（`ISSUANCE_APPROVER_ROLE` / `COMPLIANCE_ADMIN_ROLE` / `SETTLEMENT_ADMIN_ROLE` / `PAUSER_ROLE` / `UPGRADER_ROLE`）。后续角色管理通过标准 OZ AccessControl `grantRole` / `revokeRole` 完成。
...
#### 6.3.5 关键 RFQ 维护：UUPS 升级后必须刷新 EIP-712 Domain Separator
