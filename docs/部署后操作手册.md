...
flowchart TD
  ...
  Phase2 --> Phase3["Phase 3：一级认购<br/>createSubscription → subscribe"]
...
factory.approveIssuance(
    ...
    address complianceImplementation, // 合规模板地址（从 deployments JSON 获取）
...
BondConfig memory config = BondConfig({
    ...
    couponRateBps: 500,              // 票息率 bps（500 = 5%）
...
SubscriptionTerms memory terms = SubscriptionTerms({
    ...
    maxUnits: 1_000,          // 最大发行量（decimals=0 时为整数）
...
误转到 BondIssuance 合约的代币可由 `DEFAULT_ADMIN_ROLE` 取出：
...
# 一级认购
make ops-create-subscription ARGS="<bondToken> <settlementToken> <unitPrice> <maxUnits> <opensAt> <closesAt>"
make ops-subscribe ARGS="<offerId> <units>"
make ops-close-subscription ARGS="<offerId>"
make ops-update-subscription ARGS="<offerId> <bondToken> <settlementToken> <unitPrice> <maxUnits> <opensAt> <closesAt>"
