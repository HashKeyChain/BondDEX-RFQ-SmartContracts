...
abstract contract DeployConfigParser is Script {
    ...
    function _parseConfig(
        ...
        cfg.factoryAdmin = vm.parseJsonAddress(
            json,
            ".roles.bondFactory.admin"
        );
        cfg.factoryIssuanceApprover = vm.parseJsonAddress(
            json,
            ".roles.bondFactory.issuanceApprover"
        );
        cfg.factoryComplianceAdmin = vm.parseJsonAddress(
            json,
            ".roles.bondFactory.complianceAdmin"
        );
        cfg.factoryPauser = vm.parseJsonAddress(
            json,
            ".roles.bondFactory.pauser"
        );

        cfg.issuanceAdmin = vm.parseJsonAddress(
            json,
        ...
        cfg.issuancePauser = vm.parseJsonAddress(
            json,
            ".roles.bondIssuance.pauser"
        );
        cfg.issuanceUpgrader = vm.parseJsonAddress(
            json,
            ".roles.bondIssuance.upgrader"
        );

        cfg.rfqAdmin = vm.parseJsonAddress(json, ".roles.rfqSettlement.admin");
        cfg.rfqSettlementAdmin = vm.parseJsonAddress(
            json,
            ".roles.rfqSettlement.settlementAdmin"
        );
        cfg.rfqPauser = vm.parseJsonAddress(
            json,
            ".roles.rfqSettlement.pauser"
        );
        cfg.rfqUpgrader = vm.parseJsonAddress(
            json,
            ".roles.rfqSettlement.upgrader"
        );

        cfg.feeRecipient = vm.parseJsonAddress(json, ".feeConfig.feeRecipient");
        cfg.currentFeeBps = uint16(
            vm.parseJsonUint(json, ".feeConfig.currentFeeBps")
        );
    ...
    function _parseTokenPolicies(
        string memory json
    ) internal pure returns (TokenPolicy[] memory) {
        bytes memory raw = vm.parseJson(json, ".settlementTokens");
        SettlementTokenJson[] memory parsed = abi.decode(
            raw,
            (SettlementTokenJson[])
        );
        ...
        for (uint256 i = 0; i < parsed.length; i++) {
            tokens[i] = TokenPolicy({
                ...
                settlementEnabled: parsed[i]
                    .bondIssuancePolicy
                    .settlementEnabled,
                redemptionEnabled: parsed[i]
                    .bondIssuancePolicy
                    .redemptionEnabled,
    ...
    function _validateConfig(
        ...
        require(
            cfg.factoryAdmin != address(0),
            "roles.bondFactory.admin required"
        );
        require(
            cfg.factoryIssuanceApprover != address(0),
            "roles.bondFactory.issuanceApprover required"
        );
        require(
            cfg.factoryComplianceAdmin != address(0),
            "roles.bondFactory.complianceAdmin required"
        );
        require(
            cfg.factoryPauser != address(0),
            "roles.bondFactory.pauser required"
        );
        ...
        require(
            ...
            "roles.bondIssuance.settlementAdmin required"
        );
        require(
            cfg.issuancePauser != address(0),
            "roles.bondIssuance.pauser required"
        );
        require(
            cfg.issuanceUpgrader != address(0),
            "roles.bondIssuance.upgrader required"
        );

        require(
            cfg.rfqAdmin != address(0),
            "roles.rfqSettlement.admin required"
        );
        require(
            cfg.rfqSettlementAdmin != address(0),
            "roles.rfqSettlement.settlementAdmin required"
        );
        require(
            cfg.rfqPauser != address(0),
            "roles.rfqSettlement.pauser required"
        );
        require(
            cfg.rfqUpgrader != address(0),
            "roles.rfqSettlement.upgrader required"
        );

        require(
            cfg.feeRecipient != address(0),
            "feeConfig.feeRecipient required"
        );
        require(cfg.maxFeeBps <= 10_000, "maxFeeBps exceeds 100%");
        require(
            cfg.currentFeeBps <= cfg.maxFeeBps,
            "currentFeeBps > maxFeeBps"
        );

        require(tokens.length > 0, "at least one settlementToken required");
        for (uint256 i = 0; i < tokens.length; i++) {
            require(
                tokens[i].token != address(0),
                "settlementTokens[].token required"
            );
            for (uint256 j = 0; j < i; j++) {
                require(
                    tokens[i].token != tokens[j].token,
                    "duplicate settlementToken address"
                );
            }
        }
    }
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i].token == address(0)) {
                tokens[i].token = address(
                    new MockERC20Decimals("Mock USDC", "mUSDC", 6)
                );
            }
        }
    }
