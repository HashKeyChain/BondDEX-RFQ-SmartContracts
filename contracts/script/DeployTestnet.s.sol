...
contract DeployTestnet is BaseDeploy {
    /// @dev Deploys the HashKey testnet stack and records the manifest in `deployments/133.json`.
    function run() external returns (DeploymentRecord memory record) {
        uint256 deployerPrivateKey = vm.envUint(_deployerKeyEnvKey(HSK_TESTNET_CHAIN_ID));
        address deployer = vm.addr(deployerPrivateKey);
        address safeAdmin = vm.envAddress(_safeAdminEnvKey(HSK_TESTNET_CHAIN_ID));
        address settlementToken = vm.envAddress(_settlementTokenEnvKey(HSK_TESTNET_CHAIN_ID));
        ...
        console2.log("Testnet manifest updated at", _deploymentFile(HSK_TESTNET_CHAIN_ID));
