...
contract SettlementOrderEIP712Harness {
    ...
    function domainSeparator(address verifyingContract, uint256 chainId) external pure returns (bytes32) {
        return SettlementOrderEIP712.domainSeparator(verifyingContract, chainId);
    }
    ...
    function hashTypedData(Order memory order, address verifyingContract, uint256 chainId)
        ...
        returns (bytes32)
    {
        return SettlementOrderEIP712.hashTypedData(order, verifyingContract, chainId);
    }
}
...
contract SettlementOrderEIP712Test is Test {
    ...
    function test_typedHashChangesAcrossChainOrVerifyingContract() public {
        ...
        bytes32 contractHash = harness.hashTypedData(order, address(0xCAFE), 133);
