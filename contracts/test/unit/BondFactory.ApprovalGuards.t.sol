...
contract BondFactoryApprovalGuardsTest is Test {
    ...
    function test_approveIssuanceCanOverwriteRevokedApproval() public {
        vm.startPrank(admin);
        factory.approveIssuance(
            approvalId, issuer, address(complianceImplementation), block.timestamp + 1 days, metadataHash
        );
    ...
    function test_revokeIssuanceOnNonexistentApprovalSucceedsButIsHarmless() public {
        bytes32 unknown = keccak256("unknown");
        vm.prank(admin);
        factory.revokeIssuance(unknown);

        (, , ApprovalStatus status, ,) = factory.getIssuanceApproval(unknown);
